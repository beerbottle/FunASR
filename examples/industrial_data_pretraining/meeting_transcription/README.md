...
- **可插拔 LLM**：Claude (`claude-opus-4-8`) 为默认，预留 OpenAI-compatible 适配位
...
# 2. 带 LLM 纪要（需设 ANTHROPIC_API_KEY）
export ANTHROPIC_API_KEY=sk-ant-...
python run.py --audio meeting.wav --config config.example.yaml

# 或者临时开启 LLM（不改配置文件）
python run.py --audio meeting.wav   # 然后编辑 config 中 llm.enabled: true
```
...
| `llm.model` | `claude-opus-4-8` | LLM 模型 |
...
llm:
  enabled: true
  provider: "openai_compat"      # 需在 llm_summarizer.py 中填入 SDK 调用
  base_url: "https://your-api/v1"
  model: "your-model-name"
```

目前 `OpenAICompatClient` 为占位符，需在 `llm_summarizer.py` 中添加对应 SDK 调用。
